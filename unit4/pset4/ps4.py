...
def r_squared(y, estimated):
    ...
    """
    rss = ((np.array(y) - np.array(estimated))**2).sum()
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    fig, axes = pylab.subplots(ncols=len(models))
    ...
    if (len(models)) > 1:
        ...
        for model, ax in zip(models, axes):
            ...
            # plot fitted lines
            fitted_values = np.polyval(model, x)
            ax.plot(x, fitted_values, color='red', ls='-', label='model')
            # aesthetics
            r2 = r_squared(y, fitted_values)
            ax.set(xlabel='time', ylabel='temp')
            ax.set_title('degree: {}\nR2: {}'.format(len(model)+1, r2))
            ax.legend(loc='best')
    else:
        # plot observed points
        axes.scatter(x, y, color='blue', marker='o', label='data')
        # plot fitted lines
        fitted_values = np.polyval(models[0], x)
        axes.plot(x, fitted_values, color='red', ls='-', label='model')
        # aesthetics
        r2 = r_squared(y, fitted_values)
        axes.set(xlabel='time', ylabel='temp')
        axes.set_title('degree: {}\nR2: {}'.format(len(models[0]) + 1, r2))
